...
public sealed partial class SpaceArtillerySystem : EntitySystem
{
    ...
    private void OnSignalReceived(EntityUid uid, SpaceArtilleryComponent component, ref SignalReceivedEvent args)
    {
        ...
        if (apc is { Powered: true } || battery?.LastCharge >= component.PowerUseActive) //CARMINE MONOPORT: batterycomp is now shared, use lastcharge instead of currentcharge
    ...
    private void OnApcChanged(EntityUid uid, SpaceArtilleryComponent component, ref PowerChangedEvent args)
    {
        if (TryComp<BatterySelfRechargerComponent>(uid, out var batteryCharger))
        {
            if (args.Powered)
            {
                // batteryCharger.AutoRecharge = true; //CARMINE MONOPORT: this no longer exists and instead autorechargerate is set to 0 to disable
    ...
    private void OnBatteryChargeChanged(EntityUid uid, SpaceArtilleryComponent component, ref ChargeChangedEvent args)
    {
        ...
        {
            apcPowerReceiver.Load = battery.LastCharge >= battery.MaxCharge * 0.99 ? component.PowerUsePassive : component.PowerUsePassive + component.PowerChargeRate; //CARMINE MONOPORT: batterycomp is now shared, use lastcharge instead of currentcharge
        }
    }
    ...
    private void TryFireArtillery(EntityUid uid, TransformComponent xform, SpaceArtilleryComponent component)
    {
        ...
        // gun.ShootCoordinates = targetCoordinates; //CARMINE MONOPORT: gunsystem got changed and you should only change this using the A.P.I.
        // attemptshoot does this so i think it's ok for now.
        // .2 | 2026
    ...
    private void OnShotEvent(EntityUid uid, SpaceArtilleryComponent component, AmmoShotEvent args)
    {
        ...
        if (TryComp<BatteryComponent>(uid, out var battery))
        {
            ...
            // _battery.UseCharge(uid, component.PowerUseActive, battery); //carmine monoport: no need to pass the battery comp separately now
        }
    }
