...
public sealed partial class ShuttleMapControl : BaseShuttleControl
{
    ...
    [Dependency] private readonly IPrototypeManager _prototypeManager = default!; //carmine
    [Dependency] private readonly IRobustRandom _random = default!; //carmine
    ...
    // CARMINE: for biome vertexes, cache the result of the earcut triangulation calculation in this so we don't do it per-frame. vertex data dont change midgame
    // string = biomeID
    // flatCoords -> flattened coordinates obtained from the biome's vertex coordinate vector2 YAML
    // triangles -> indices of triangles in flatCoords, obtained from Earcut.Tesselate.
    Dictionary<string, (float[] flatCoords, List<int> triangles)> _biomeTriangulationCache = new();

    // CARMINE: determines amount of wiggle for biome edges
    private const float WiggleAmount = 10f;
    ...
    protected override void Draw(DrawingHandleScreen handle)
    {
        ...
        // CARMINE - draw biomes by .2 | 2026
        // how does this work?
        // each ambientSpaceBiome has VertexCoordinates that, clockwise, define a polygon of biome area.
        // the EARCUT algorithm (look it up) is a triangulation algorithm that takes a polygon's set of vertices (clockwise) and
        // slices it in triangles (ears). we need this because the engine needs said triangles to draw a polygon
        //
        // so
        // for each biome proto, if the vertex coords ain't null
        // grab all vertex pairs & flatten them into a single float array
        // feed said float array into the EarCut algorithm to get another array that defines triangles
        //

        var biomeProtos = _prototypeManager.EnumeratePrototypes<SpaceBiomePrototype>();
        foreach (var biome in biomeProtos)
        {
            if (biome.VertexCoordinates.Count == 0) //if the biome has no coords then we don't bother doing anything
                continue;
            var flattenedCoords = new float[biome.VertexCoordinates.Count * 2];
            var tesselationResult = new List<int>();
            // Get triangle indices from earcut
            // in ShuttleMapControl.xaml.cs we cache the triangulation result, because the actual coordinates we want
            // are UI coordinates and those need to be live calculated (cuz transform + scalePosition)
            // we also cache flattenedCoords, even if it's not much to calculate we might as well
            if (!_biomeTriangulationCache.TryGetValue(biome.ID, out var cached)) //if we haven't previously calculated flattenedCoords & tesselation result, do it & cache
            {
                int i = 0;
                foreach (var coordPair in biome.VertexCoordinates)
                {
                    // Flatten markers into the [x0,y0, x1,y1, ...] format earcut expects
                    flattenedCoords[i * 2] = coordPair.X;
                    flattenedCoords[i * 2 + 1] = coordPair.Y;
                    i++;
                }
                tesselationResult = Earcut.Tessellate(flattenedCoords.Select(f => (double)f).ToList(), []);
                _biomeTriangulationCache[biome.ID] = (flattenedCoords, tesselationResult);  //tesselationResult: this is in format like [ 1 0 2 2 2 4 1 2 3 ]
                                                                                            //that is the indices of the POINTS forming a triangle. 1 0 2 | 0 2 3 | 1 2 3 being
                                                                                            //3 triangles.
                                                                                            // 1st 0nd 2nd points - triangle 1
                                                                                            // 0nd 2nd 3rd points - triangle 2
                                                                                            // 1st 2nd 3rd points - triangle 3
                                                                                            // this example isnt a valid triangle set but you get the idea
            }
            else
            {
                flattenedCoords = cached.flatCoords;
                tesselationResult = cached.triangles;
            }
            // from the top info, convert indices back to a list of coordinates / vertices that TriangleList wants + do the matrix stuff to it
            var triangleVerts = new Vector2[tesselationResult.Count];
            for (int j = 0; j < tesselationResult.Count; j++)
            {
                int idx = tesselationResult[j];
                var triangle_pos = ApplyMatrixForMinimap(new Vector2(
                                                        flattenedCoords[idx * 2],
                                                        flattenedCoords[idx * 2 + 1]),
                                                        matty);
                triangleVerts[j] = triangle_pos;
            }

            var wiggledBorder = new List<Vector2>();
            foreach (var coordPair in biome.VertexCoordinates)
            {
                // Flatten markers into the [x0,y0, x1,y1, ...] format earcut expects
                wiggledBorder.Add(
                    ApplyMatrixForMinimap(
                        new Vector2(
                        coordPair.X + _random.NextFloat(-WiggleAmount / 2, WiggleAmount / 2),
                        coordPair.Y + _random.NextFloat(-WiggleAmount / 2, WiggleAmount / 2)
                        ), matty));
            }

            Color biomeColor = biome.BiomeColor.WithAlpha(biome.BiomeAlpha);

            //getting pos for drawing title of biome
            if (biome.TitleCoordinates != null)
            {
                var title_pos = ApplyMatrixForMinimap((Vector2)biome.TitleCoordinates, matty);
                handle.DrawString(_font, title_pos, biome.Name, biomeColor.WithAlpha(1f));
            }
            handle.DrawPrimitives(DrawPrimitiveTopology.LineLoop, wiggledBorder, biomeColor.WithAlpha(1f)); //draw outside line. this also draws the triangles inside but i think that looks badass .2 | 2026
            handle.DrawPrimitives(DrawPrimitiveTopology.TriangleList, triangleVerts, biomeColor); //draw inside filling
        }
    ...
    /// <summary>
    /// helper function to flip Y, apply matrix, and scale it for displaying coords on map screen. keeps code a little cleaner
    /// </summary>
    /// <param name="inputVector"></param>
    /// <param name="matrix"></param>
    private Vector2 ApplyMatrixForMinimap(Vector2 inputVector, Matrix3x2 matrix)
    {
        var temp_pos = Vector2.Transform(inputVector, matrix);
        temp_pos = temp_pos with { Y = -temp_pos.Y };
        var final_pos = ScalePosition(temp_pos);
        return final_pos;
    }
}
